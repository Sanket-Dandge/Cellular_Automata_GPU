...
__global__ void forest_fire (int dim, CELL_TYPE *grid, CELL_TYPE *newGrid)
{
    /*
    ...
    */
    ...
    int numFireNeighbors;
 
    if ( (iy>0 && iy < dim+1) && (ix>0 && ix < dim+1) ) {
 
        // Get the number of neighbors for a given grid point
        numFireNeighbors = (grid[id+(dim+2)]==FIRE) + (grid[id-(dim+2)]==FIRE)  // upper lower
                     + (grid[id+1]==FIRE) + (grid[id-1]==FIRE);                 // right left
 
        int cell = grid[id];
        // Here we have explicitly all of the game rules
        if (cell == EMPTY)
            newGrid[id] = EMPTY;
        else if (cell == ASH && numFireNeighbors > 0)
            newGrid[id] = ASH;
        else if (cell == ASH && numFireNeighbors == 0)
            newGrid[id] = EMPTY;
        else if (cell == TREE && numFireNeighbors > 0)
            newGrid[id] = FIRE;
        else if (cell == TREE && numFireNeighbors == 0)
            newGrid[id] = TREE;
        else if (cell == FIRE)
...
int forest_fire_baseline(int timesteps)
{
    ...
    #endif
        forest_fire<<<gridSize, blockSize>>>(dim, d_grid, d_newGrid);
