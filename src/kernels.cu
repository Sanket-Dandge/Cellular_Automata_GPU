...
namespace kernels {
  ...
  __global__ void simpleGhostNextGenerationKernel(bool* currentGrid, bool* nextGrid, int N) {
    ...
    if ((col < N + 1) && (row < N + 1)) {
      ...
      int livingNeighbors = kernels::countNeighbors(currentGrid, col, left, right, center, up, down);
      nextGrid[index] = livingNeighbors == 3 || (livingNeighbors == 2 && currentGrid[index]) ? 1 : 0;
    }
    return;
  }
  ...
  __global__ void updateColorArray(GLubyte* colorArray, bool *currentGrid, bool *nextGrid, int N) {
    ...
    for(int i = indexY; i < N + 1; i+=strideY) {
      ...
      for(int j = indexX; j < N + 1; j += strideX) {
        ...
        if (nextGrid[index]) {
          colorArray[colorIndex + 2] >= 255 ? colorArray[colorIndex + 2] = 255 : colorArray[colorIndex + 2]++;
        }
      }
    }
  }
